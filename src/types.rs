...
// pub fn matches_target<T>(tiles: &[[Option<T>; 5]; 5], target: &Target) -> bool
// where
//     T: Into<Color>,
// {
//     for (board_row, target_row) in tiles[1..=3].iter().zip(target) {
//         for (tile, target_color) in board_row[1..=3].iter().zip(target_row) {
//             if !tile
//                 .map(|tile| tile.into() == *target_color)
//                 .unwrap_or(false)
...
//         for (board_row, target_row) in self.tiles[1..=3].iter().zip(target) {
//             for (tile, target_color) in board_row[1..=3].iter().zip(target_row) {
//                 if !tile.map(|tile| tile == *target_color).unwrap_or(false) {
//                     return false;
//                 }
//             }
//         }
